<template>
  <div class="dashboard-container">
    <i v-if="checkIfCanEdit() === true" class="edit-button pointer pi pi-cog" @click="openEdit()"></i>
      ...
      <div class="content">
        <div class="grid grid-cols-3 gap-4">
          <div v-for="(snr, index) of sensors" :class="['slot', { 'error-text': snr.status >= 0 && snr.status <= 3, 'flex-center': snr.status < 5 }]">
            ...
            <ErrorRetry v-else-if="snr.status === 2" message="pages.dashboard-view.error2" :title="snr.name" @reload="fetchMeasurementsRetry(index)" />
            <ErrorRetry v-else-if="snr.status === 3" message="pages.dashboard-view.error3" :title="snr.name" @reload="fetchMeasurementsRetry(index)" />
            <ErrorRetry v-else-if="snr.status === 4" message="pages.dashboard-view.error4" :title="snr.name" @reload="fetchMeasurementsRetry(index)" />
...
watch(() => props.dashboard, (newValue) => {
  if (newValue !== '') {
...
const selectedYear = ref(date.getFullYear()); // Selected year, defaults to current year
...
onMounted(() => {
  userRole = StorageGet('role'); // User role
...
/**
 * Check if the user can edit the dashboard
 * @returns True if the user can edit, false otherwise
 */
const checkIfCanEdit = (): boolean => {
  return ((props.dashboard === 'user') || (userRole === 'appadmin'));
}
...
const fetchMeasurements = async (startTime: string, endTime: string): Promise<void> => {
  ...
  if (dataFetch.max > 0) {
    for (const s of sid) {
      const apiUrl = `${AppSettings.env.API_URL}/measurements/${s}`;
      axios
